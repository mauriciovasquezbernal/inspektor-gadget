...
struct event {
	struct gadget_endpoint_t src;
	struct gadget_endpoint_t dst;
...
trace_v4(struct pt_regs *ctx, pid_t pid, struct sock *sk, __u16 dport, __u64 mntns_id)
{
	...
	BPF_CORE_READ_INTO(&event.src.addr.v4, sk, __sk_common.skc_rcv_saddr);
	BPF_CORE_READ_INTO(&event.dst.addr.v4, sk, __sk_common.skc_daddr);
	event.dst.port = bpf_ntohs(dport); // host expects data in host byte order
...
trace_v6(struct pt_regs *ctx, pid_t pid, struct sock *sk, __u16 dport, __u64 mntns_id)
{
	...
	BPF_CORE_READ_INTO(&event.src.addr.v6, sk,
			   __sk_common.skc_v6_rcv_saddr.in6_u.u6_addr32);
	BPF_CORE_READ_INTO(&event.dst.addr.v6, sk,
			   __sk_common.skc_v6_daddr.in6_u.u6_addr32);
	event.dst.port = bpf_ntohs(dport); // host expects data in host byte order
	event.src.port = BPF_CORE_READ(sk, __sk_common.skc_num);;
	bpf_get_current_comm(event.task, sizeof(event.task));
...
static __always_inline int handle_tcp_rcv_state_process(void *ctx, struct sock *sk)
{
	...
	if (family == AF_INET) {
		event.src.version = event.dst.version = 4;
		event.src.addr.v4 = BPF_CORE_READ(sk, __sk_common.skc_rcv_saddr);
		event.dst.addr.v4 = BPF_CORE_READ(sk, __sk_common.skc_daddr);
	} else {
		event.src.version = event.dst.version = 6;
		BPF_CORE_READ_INTO(&event.src.addr.v6, sk,
				__sk_common.skc_v6_rcv_saddr.in6_u.u6_addr32);
