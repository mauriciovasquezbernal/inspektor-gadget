			...
			Validator: func(value string) error {
				...
				for _, node := range nodes {
					if _, ok := nodeMap[node]; ok {
						return fmt.Errorf("duplicate node: %s", node)
					}
					nodeMap[node] = struct{}{}
				}
				return nil
...
func (r *Runtime) RunGadget(gadgetCtx runtime.GadgetContext) (map[string][]byte, error) {
	...
	if len(pods) == 0 {
		return nil, fmt.Errorf("Inspesktor Gadget is not running on the requested node(s): %v", nodes)
	}
	...
	if gadgetCtx.GadgetDesc().Type() == gadgets.TypeOneShot {
		print := gadgetCtx.Parser().EnableCombiner(len(pods))
		defer print()
	}
	...
	for _, pod := range pods {
		...
		go func(pod gadgetPod) {
			...
			if err != nil {
				gadgetCtx.Logger().Errorf("node %q: %v", pod.node, err)
			}
			if res != nil {
				resultsLock.Lock()
...
func (r *Runtime) runGadget(gadgetCtx runtime.GadgetContext, pod gadgetPod) ([]byte, error) {
	...
	go func() {
		...
		for {
			...
			case pb.EventTypeGadgetDone:
				gadgetCtx.Logger().Debugf("%-20s | got EventTypeGadgetDone from server", pod.node)
				doneChan <- true
				return
			default:
	...
	select {
	case <-doneChan:
		gadgetCtx.Logger().Debugf("%-20s | done from server side", pod.node)
	case <-gadgetCtx.Context().Done():
		...
		select {
		case <-doneChan:
			gadgetCtx.Logger().Debugf("%-20s | done after cancel request", pod.node)
