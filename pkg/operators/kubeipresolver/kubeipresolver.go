...
// KubeNetworkInformation is for now a specific interface for trace network
// TODO: More granualar and "shareable" interfaces
...
func (k *KubeIPResolver) CanOperateOn(gadget gadgets.GadgetDesc) bool {
	...
	_, hasAdditionalInf := gadget.EventPrototype().(KubeNetworkInformation)
	return hasAdditionalInf
}
...
func (k *KubeIPResolver) Instantiate(gadgetCtx operators.GadgetContext, gadgetInstance any, params *params.Params) (operators.OperatorInstance, error) {
	traceInstance := &KubeIPResolverInstance{
		gadgetCtx:      gadgetCtx,
		manager:        k,
