...
type Operator interface {
	...
	// CanOperateOnRun tests whether the operator supports the given containerized gadget.
	CanOperateOnRun(*runTypes.GadgetInfo) bool
...
// GetOperatorsForRunGadget checks which operators can work with the given containerized gadget and
// returns a collection of them
func GetOperatorsForRunGadget(info *runTypes.GadgetInfo) Operators {
	out := make(Operators, 0)
	for _, operator := range allOperators {
		if operator.CanOperateOnRun(info) {
			out = append(out, operator)
		}
	}
	out, err := SortOperators(out)
