...
type Parser interface {
	...
	AggregateEntries(cols []string, entries any) (map[string]*AttrsCount, error)
	...
	for _, colName := range colNames {
		...
		case reflect.Int:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[int](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Int8:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[int8](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Int16:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[int16](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Int32:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[int32](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Int64:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[int64](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Uint:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[uint](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Uint8:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[uint8](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Uint16:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[uint16](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Uint32:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[uint32](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.Uint64:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[uint64](a, offset)
				return attribute.Int64Value(int64(rawVal))
			}
		case reflect.String:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[string](a, offset)
				return attribute.StringValue(rawVal)
			}
		case reflect.Float32:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[float32](a, offset)
				return attribute.Float64Value(float64(rawVal))
			}
		case reflect.Float64:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[float64](a, offset)
				return attribute.Float64Value(rawVal)
			}
		case reflect.Bool:
			getter = func(a *T) attribute.Value {
				rawVal := columns.GetField[bool](a, offset)
				return attribute.BoolValue(rawVal)
			}
		default:
...
func (p *parser[T]) AggregateEntries(cols []string, entries any) (map[string]*AttrsCount, error) {
	counters := make(map[string]*AttrsCount)
	getter, err := p.AttrsGetter(cols)
	if err != nil {
		return nil, err
	}
...
func (p *parser[T]) ColIntGetter(colName string) (func(any) int64, error) {
	...
	var getter func(any) int64

	switch col.Kind() {
	case reflect.Int:
		getter = func(a any) int64 {
			rawVal := columns.GetField[int](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Int8:
		getter = func(a any) int64 {
			rawVal := columns.GetField[int8](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Int16:
		getter = func(a any) int64 {
			rawVal := columns.GetField[int16](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Int32:
		getter = func(a any) int64 {
			rawVal := columns.GetField[int32](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Int64:
		getter = func(a any) int64 {
			rawVal := columns.GetField[int64](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Uint:
		getter = func(a any) int64 {
			rawVal := columns.GetField[uint](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Uint8:
		getter = func(a any) int64 {
			rawVal := columns.GetField[uint8](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Uint16:
		getter = func(a any) int64 {
			rawVal := columns.GetField[uint16](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Uint32:
		getter = func(a any) int64 {
			rawVal := columns.GetField[uint32](a.(*T), offset)
			return int64(rawVal)
		}
	case reflect.Uint64:
		getter = func(a any) int64 {
			rawVal := columns.GetField[uint64](a.(*T), offset)
			return int64(rawVal)
		}
	default:
		return nil, fmt.Errorf("we're not handling this type yet")
	}
...
func (p *parser[T]) ColFloatGetter(colName string) (func(any) float64, error) {
	...
	var getter func(any) float64

	switch col.Kind() {
	case reflect.Float32:
		getter = func(a any) float64 {
			rawVal := columns.GetField[float32](a.(*T), offset)
			return float64(rawVal)
		}
	case reflect.Float64:
		getter = func(a any) float64 {
			rawVal := columns.GetField[float64](a.(*T), offset)
			return float64(rawVal)
		}
	default:
		return nil, fmt.Errorf("we're not handling this type yet")
