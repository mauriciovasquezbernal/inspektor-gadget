...
func (n *ContainerNotifier) install() error {
	...
	consts := map[string]interface{}{
		"tracer_fanotify_fd": uint64(runtimeBinaryNotify.Fd),
	}
	...
	l, err := link.Kprobe("fsnotify_remove_first_event", n.objs.IgFaPick, nil)
	if err != nil {
...
func (n *ContainerNotifier) watchRuntimeIterate() (bool, error) {
	...
	callerCommArr := strings.Split(string(record.CallerComm[:]), "\x00")
	callerComm := callerCommArr[0]

	cmdlineArr := []string{}
	calleeComm := ""
	for _, arg := range strings.Split(string(record.Args[:]), "\x00") {
		if arg != "" {
