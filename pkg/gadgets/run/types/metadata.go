...
func doPopulateDatasourceFields(ds *metadatav1.DataSource, btfStruct *btf.Struct, prefix string,
	existingFields map[string]struct{}) error {
	if ds.Fields == nil {
	...
	for _, member := range btfStruct.Members {
		...
		if t, ok := member.Type.(*btf.Struct); ok {
			if err := doPopulateDatasourceFields(ds, t, prefix+member.Name+".", existingFields); err != nil {
				return fmt.Errorf("populating struct: %w", err)
			}
...
func populateDatasourceFields(ds *metadatav1.DataSource, btfStruct *btf.Struct) error {
	...
	return doPopulateDatasourceFields(ds, btfStruct, "", existingFields)
}
