	...
	for name, test := range map[string]testDefinition{
		...
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				// Validate error. Use assert instead of require to avoid
				// blocking the main goroutine.
				if test.expectedResult.err {
					assert.NotNil(t, err, "expected error running tracer")
				} else {
					assert.Nil(t, err, "not expected error running tracer")
				}
			...
			// Get the result. If we expect an error, we don't need to validate
			data := <-out
			if test.expectedResult.err {
				require.Nil(t, data, "available data when error occurred")
				return
			}
			...
			if test.expectedResult.exactHistograms != 0 {
				...
				require.GreaterOrEqual(t, lHis, test.expectedResult.minHistograms,
					"expecting at least %d histograms, got %d", test.expectedResult.minHistograms, lHis)
			}
			...
			for _, h := range result.Histograms {
				...
				require.Equal(t, expectedUnit, h.Unit,
					"expecting %s unit, got %s", expectedUnit, h.Unit)
