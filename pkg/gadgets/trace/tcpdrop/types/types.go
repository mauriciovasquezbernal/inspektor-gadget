...
type Event struct {
	...
	Saddr string `json:"saddr,omitempty" column:"saddr,template:ipaddr,order:2001"`
	Sport uint16 `json:"sport,omitempty" column:"sport,template:ipport,order:2002"`

	Daddr string `json:"daddr,omitempty" column:"daddr,template:ipaddr,order:3001"`
	Dport uint16 `json:"dport,omitempty" column:"dport,template:ipport,order:3002"`
	...
	SrcKind      eventtypes.RemoteKind `json:"srcKind,omitempty" column:"srcKind,maxWidth:5,order:2100"`
	SrcNamespace string                `json:"srcNamespace,omitempty" column:"srcns,order:2101"`
	SrcName      string                `json:"srcName,omitempty" column:"srcname,order:2102"`

	/* Destination IP resolved by kubeipresolver  */
	DstKind      eventtypes.RemoteKind `json:"dstKind,omitempty" column:"dstKind,maxWidth:5,order:3100"`
	DstNamespace string                `json:"dstNamespace,omitempty" column:"dstns,order:3101"`
	DstName      string                `json:"dstName,omitempty" column:"dstname,order:3102"`
}
...
func (e *Event) SetNode(node string) {
	e.Node = node
}
...
func GetColumns() *columns.Columns[Event] {
	...
	// Hide columns already shown in virtual columns
	hide := []string{"saddr", "daddr", "Sport", "Dport", "SrcKind", "srcns", "SrcName", "DstKind", "dstns", "DstName"}
	for _, h := range hide {
		col, ok := cols.GetColumn(h)
		if !ok {
			panic("column " + h + " not found")
		}
		col.Visible = false
	}
