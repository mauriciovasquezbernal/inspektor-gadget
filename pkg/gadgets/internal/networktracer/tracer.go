...
type attachment struct {
	dispatcherSpec       *ebpf.CollectionSpec
	dispatcherCollection *ebpf.Collection
	dispatcherProg       *ebpf.Program
...
func (t *Tracer[Event]) newAttachment(
	...
	defer func() {
		if err != nil {
			...
			if a.dispatcherCollection != nil {
				a.dispatcherCollection.Close()
			}
		}
	}()

	a.dispatcherSpec, err = loadDispatcher()
	if err != nil {
		return nil, err
	}
	...
	if err := a.dispatcherSpec.RewriteConstants(consts); err != nil {
		return nil, fmt.Errorf("RewriteConstants while attaching to pid %d: %w", pid, err)
	}
	a.dispatcherSpec.Maps["tail_call"].Contents = []ebpf.MapKV{
		{
			Key:   uint32(0),
	...
	a.dispatcherCollection, err = ebpf.NewCollection(a.dispatcherSpec)
	if err != nil {
		return nil, fmt.Errorf("creating BPF collection: %w", err)
	}

	var ok bool
	a.dispatcherProg, ok = a.dispatcherCollection.Programs["ig_net_disp"]
	if !ok {
		return nil, fmt.Errorf("dispatcher BPF program not found")
	}
	...
	if err := syscall.SetsockoptInt(a.sockFd, syscall.SOL_SOCKET, unix.SO_ATTACH_BPF, a.dispatcherProg.FD()); err != nil {
