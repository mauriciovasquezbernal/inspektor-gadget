...
type testCase struct {
	...
	addr          string
	port          int
	expectedEvent func(info *utilstest.RunnerInfo, e extraInformation) *types.Event
...
func TestSnapshotSocket(t *testing.T) {
	...
	cases := []testCase{
		{
			...
			expectedEvent: func(info *utilstest.RunnerInfo, e extraInformation) *types.Event {
				return &types.Event{
					...
					WithNetNsID: eventtypes.WithNetNsID{NetNsID: e.netNsID},
					Protocol:    "TCP",
					Status:      "LISTEN",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			utilstest.ExpectAtLeastOneEvent(c.expectedEvent)(t, nil, extraInformation{
				addr:    c.addr,
				port:    uint16(c.port),
				netNsID: runner.Info.NetworkNsID,
			},
				events)
		})
	}
