...
type BuildGadgetImageOpts struct {
	...
	CreatedDate string
	// Path to help binary when the image is run by mistake with "docker run" or similar command.
	// It must be a tar file containing a binary named help.
	HelpPath string
}
...
func createImageIndex(ctx context.Context, target oras.Target, o *BuildGadgetImageOpts) (ocispec.Descriptor, error) {
	...
	for _, arch := range archs {
		...
		hostManifestDesc, err := createManifestForHostTarget(ctx, target, o.HelpPath)
		if err != nil {
...
func createManifestForHostTarget(ctx context.Context, target oras.Target, layerPath string) (ocispec.Descriptor, error) {
	layerDescs := []ocispec.Descriptor{}
	...
	configDesc, err := createConfigHostDesc(ctx, target, binaryDesc)
	if err != nil {
		return ocispec.Descriptor{}, fmt.Errorf("creating empty descriptor: %w", err)
	}
...
func createConfigHostDesc(ctx context.Context, target oras.Target, layerDesc ocispec.Descriptor) (ocispec.Descriptor, error) {
